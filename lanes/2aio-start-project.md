---
description: 2AIO取締役会(CEO方針→CMO/CTO並列→CEO統合)を開催しPRDを生成する。--lite で軽量モード(CTO+CEO+PRDのみ)
...
`--lite` では **CMO・2aio-researcher・2aio-r-\* を一切起動しない**（冗長排除）。フル取締役会は新規事業検討のときだけ（`--lite` なし）に使う。
...
1. **テーマ略称を冒頭で確定する**（従来 Phase 5 で行っていた生成を前倒し。ルールは同じ: `output/board-meeting-*.md` を一覧し、ファイル冒頭の「**テーマ:**」行との意味的一致があればその略称を再利用、無ければ英小文字ケバブ20文字以内で新規生成。Phase 5 では確定済み略称をそのまま使う）
...
3. **各 Phase 完了時**、中間成果を `output/{project}/board-work/` に逐次保存し board-state の `phase` を進める:
   `ceo-brief.md`（Phase 1）→ `cmo|cto-report.md`（Phase 2）→ `ceo-final.md`（Phase 3）
...
### Phase 1: CEO ブリーフ生成
オーケストレーター（本体セッション）が `2aio-ceo.md` の [Phase 1 — ブリーフ生成時] フォーマットに従い直接ブリーフを生成する（エージェント起動不要。opus 起動は Phase 3 の1回に温存）。

### Phase 2a: CMO・CTO 並列起動
`2aio-cmo`・`2aio-cto` エージェントを**単一のメッセージで2つ同時にTaskとして起動してください（並列必須）**。

- **1つずつ順番に実行することは禁止します。必ず2つを同時に発行してください。**
- 各エージェントにはCEOブリーフの該当部分を渡してください
- CMO には「最新データが必要なら『## 調査依頼リスト』を出力して一旦終了する」よう明示指示してください
- 2つすべてのTaskが完了するまで待ってから次に進んでください
...
### Phase 2c: CMO 再起動
CMO を「CEOブリーフの該当部分 + Phase 2b の集約結果（要約+統合ソース一覧。r-* の生出力全文は渡さない）」込みで再起動し、最終レポートを作成させてください。
...
- `rejected` の場合: PRD は生成せず、却下理由と再検討条件をレポートに明記して Phase 5 へ進む。
- `needs_review` の場合: CEO 統合判断の「不足情報」セクションに列挙された項目を該当役員（CMO/CTO）に1回だけ追加調査させ、Phase 3 を再実行する（最大1回。2回目も needs_review ならレポートに未解決事項を記録して終了し、ユーザーに判断を仰ぐ）。

### Phase 5: レポート出力・差分チェック
...
|---|-------------|---------|----------------|---------|
| 1 | {URL または 学習データ} | {YYYY-MM-DD} | {CMO} | {市場規模など} |

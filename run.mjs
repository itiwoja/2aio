...
import { webSearch, searchBackend, primarySources } from './lib/search.mjs';
import { claudeUsageBlocking } from './lib/ccusage.mjs';
import { usedFraction } from './lib/governor.mjs';
...
async function collect(topic) {
  const docs = [];
  // #20 第2弾: 一次ソース(GitHub Releases / npm registry)を先頭に置く — 正確な版・移行手順の根拠
  if (Array.isArray(topic.primary) && topic.primary.length) {
    const prim = await primarySources(topic.primary, { maxChars: CFG.search.maxCharsPerResult });
    for (const r of prim) docs.push({ q: '(primary)', ...r });
    log(`  一次ソース: ${prim.length}件`);
  }
...
async function audit(topic, draft, docs) {
  if ((CFG.auditBackend || 'local') === 'claude') {
    // #20(5) ガバナー連動: サブスク使用率が閾値超過なら auditLocal へ自動降格
    // （「開発を主・Forge を従」をコードで保証）。ccusage 未取得(null)は fail-open で claude 継続
    // — CONTROL-PLANE.md の既存設計に合わせる。
    try {
      const usage = await claudeUsageBlocking();
      const pct = usedFraction({ active: usage?.active || null, tokenLimit: CFG.claudeMax5x?.tokenLimit || 0 });
      const threshold = CFG.governor?.tokenThreshold ?? 0.8;
      if (pct !== null && pct >= threshold) {
        log(`  ⤵ claude監査→ローカルに降格（サブスク使用率 ${(pct * 100).toFixed(0)}% ≥ ${(threshold * 100).toFixed(0)}%）`);
        return auditLocal(topic, draft, docs);
      }
    } catch { /* 取得失敗も fail-open で claude 監査継続 */ }
    return auditClaude(topic, draft, docs);
  }
  return auditLocal(topic, draft, docs);
}
...
async function runTopic(topic) {
  ...
  // #20 第3弾: LLM 監査の前に決定的検証（コードフェンス構文 = issues として revise 注入 /
  // URL 生存確認 = uncertainties に格下げ。開発知識は「コピペで動くか」が価値の大半）
  const v0 = await verifyDraft(draft.updated_markdown).catch(() => ({ issues: [], uncertainties: [] }));
  if (v0.uncertainties.length) draft.uncertainties = [...(draft.uncertainties || []), ...v0.uncertainties];
  if (v0.issues.length) {
    log(`  検証NG→改稿: ${v0.issues.slice(0, 2).join(' | ')}`);
    const rev0 = await revise(topic, draft, v0.issues, docs);
    if (rev0?.updated_markdown) draft = rev0;
  }
  ...
  while (!a.pass && round < CFG.auditRounds) {
    ...
    // 改稿後もコードフェンスの決定的検証を維持（新たな構文エラーの混入防止）
    const vr = await verifyDraft(draft.updated_markdown).catch(() => ({ issues: [], uncertainties: [] }));
    if (vr.issues.length) a = { pass: false, issues: vr.issues };
    else a = await audit(topic, draft, docs);
  }

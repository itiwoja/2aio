...
```
2AIO
├─ agents/ commands/      … エージェント 25 体 + 入口 3 コマンド（create / check / dev）
...
```
`agents/`（25 体）`commands/`（入口 3 個）`lanes/`（内部レーン 10 本 → `~/.claude/2aio/lanes/`）`skills/`（66 個 + 入口3個の Skill 版）を配備します（**既存スキルは上書きしません** — ECC セーフ）。
...
`/2aio-` と打って補完に **`/2aio-create` `/2aio-check` `/2aio-dev` の 3 つ**が出れば導入成功です。使うのはこの 3 つだけです:
```bash
/2aio-create "ポモドーロタイマー PWA" --quick   # 一から作る（--quick で最短 1 本、省略時は規模を自動判定）
/2aio-check .                                   # 既存プロジェクトを評価 → 承認後に修正 or Issue起票まで
/2aio-dev . "CSVエクスポート機能を追加"          # 既存プロジェクトに機能追加・バグ修正（--fixでバグ修正）
```

**Codex App/CLI からも同じ3つを呼べます**（`~/.codex` がある状態でインストール済みなら）:
`/skills` メニューまたは `$2aio-create` / `$2aio-check` / `$2aio-dev` で明示呼び出し。詳細と host 差分は
...
**強い agent 集を意識しなくても使える**のが 2AIO のコンセプト。ユーザー向けコマンドは 3 つだけで、
規模判定・レーン選択・エージェント編成はすべて内部で自動化されます。

```bash
/2aio-create "沖縄観光 AI 案内チャットボット"   # ① 一から作る — 小規模なら即実装、大テーマなら取締役会→PRD→計画→実装まで自動
/2aio-check .                                   # ② 既存プロジェクトの評価 — 多観点監査→スコア付きレポート→承認後に修正 or Issue起票
/2aio-dev . "予約リマインダーを追加"             # ③ 既存プロジェクトで機能実装・バグ修正 — 最短で branch + commit まで
```

- `/2aio-create` は `--quick`（即実装を強制）/ `--full`（PRD からのフルコースを強制）で判定を上書き可。
- `/2aio-check` は `--report-only` で評価レポートのみ（コードに触らない）。承認ステップで「Issueとして起票するだけ」も選べる。
- `/2aio-dev` は `--fix "{バグ報告}"` でバグ修正モード、`--pr` で push + PR 作成まで（既定はローカル commit のみ）。

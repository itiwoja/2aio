...
// worker(claude/codex)自身の認証に必要で、秘密でも継承させ続ける key。config.worker.envKeep で上書き可。
export const WORKER_ENV_KEEP = /^(ANTHROPIC_|CLAUDE_)/i;

/**
 * 子プロセス(worker)へ継承させる env を作る（非破壊）。control plane が持つ無関係な秘密
 * (LINEAR_API_KEY 等)を落とし、非秘密(PATH/HOME 等)は素通し、worker 認証(keep)は保持する。
 * denylist 方式なので「worker が必要とする非秘密 env」を誤って壊さない。
 */
export function scrubEnv(env = {}, { keep = WORKER_ENV_KEEP } = {}) {
  const out = {};
  for (const [k, v] of Object.entries(env)) {
    if (keep.test(k)) { out[k] = v; continue; }
    if (isSecretEnvKey(k)) continue; // 無関係な秘密は worker に渡さない
    out[k] = v;
  }
  return out;
}

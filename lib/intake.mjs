...
// レーンは slash command ではなく ~/.claude/2aio/lanes/ 配下のレーン定義ファイル。
// headless ジョブへ渡すため絶対パスで埋め込む（~ をプロンプトに残さない）。
export function laneInvocation(laneName, argsText = '') {
  if (!/^2aio-[a-z0-9-]+$/.test(laneName)) throw new Error(`不正なレーン名: ${laneName}`);
  const lanePath = path.join(os.homedir(), '.claude', '2aio', 'lanes', `${laneName}.md`);
  return `${lanePath} を Read し、その指示に $ARGUMENTS を「${String(argsText).trim()}」として厳密に従って実行してください。`;
}
...
export function briefToPlanPrompt(brief, repo) {
  ...
  const lanePath = path.join(os.homedir(), '.claude', '2aio', 'lanes', '2aio-plan-project.md');
  return `次の要件（ヒアリング結果）を PRD とみなし、「${label}」の実装計画書を ${lanePath} の --lite モード相当（タスクWBS＋依存＋スプリント1行）で output/ に impl-plan-*.md として生成してください。実装はしない（後続ジョブが行う）。\n\n=== 要件 ===\n${brief}`;
}
export const IMPLEMENT_CHAIN_PROMPT = laneInvocation('2aio-implement-project', 'latest --auto');

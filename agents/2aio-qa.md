...
- **UIスモーク（UI 成果物の場合・恒常基準）**: Bash 内で `node ~/.claude/2aio/scripts/ui-smoke.mjs {URL または dist/プロジェクトdir} --out output/{project}/screenshots` を実行する（html スタックはディレクトリを渡す — スクリプトが静的サーバ経由で検証し file:// を使わない）。

...
- 計画書の受け入れ条件にない品質基準を勝手に追加しない（例: 「もっとテストを書け」「リファクタしろ」は禁止）。例外: カバレッジ 80% 既定、「lint error>0 は Fail」（warning は記録のみ・非ブロック）、および UI 成果物の UIスモーク（未捕捉例外0＋主要要素 visible）はユーザールール由来の恒常基準であり、追加扱いしない。
...
- **UIスモーク（UI 成果物の場合・恒常基準）**: Bash 内で `node C:/Users/1kkim/projects/dev/skills/2aio/scripts/ui-smoke.mjs {URL または dist/プロジェクトdir} --out output/{project}/screenshots` を実行する（html スタックはディレクトリを渡す — スクリプトが静的サーバ経由で検証し file:// を使わない）。
  - **合格条件（必須）**: 未捕捉例外 0 件 ＋ 主要要素 visible。その他の console error は警告記録に留める（サードパーティ由来の誤 Fail 防止）
  - スクリーンショット（320/1440px）のパスを qa-report.md に記録
  - Playwright 未導入（exit 3）は `[TOOL_MISSING]` 記録＋UIスモーク省略で degraded 扱い（Fail にしない）。初回セットアップ: `npm i -D playwright && npx playwright install chromium`
  - ECC `e2e-runner` は **agent 起動せずパターン参照のみ**（サブエージェント非連鎖原則）。claude-in-chrome は本エージェントの tools に無いため使用不可（interactive 時にオーケストレーターが行う代替手段）

---
description: 複数テーマの CCC 全フェーズ（取締役会→計画→実装→デプロイ）を連続自律実行するバッチオーケストレーター
argument-hint: '--themes="..." | --themes-file=... | --resume={batch_id} [--board=lite|full] [--platform=...]'
disable-model-invocation: true
---

複数のテーマについて、CCC の全フェーズ（取締役会 → 計画 → 実装 → デプロイ）を **一切止まらず** 連続実行してください。

**入力:** $ARGUMENTS

**出力先:** `output/` の正本は `C:/Users/1kkim/projects/ccc-output/` に固定（cwd に依存しない）
...
```
/ccc-autorun-batch --themes-file=batch-themes.md
```
...
For each theme in themes:
  1. /ccc-start-project {theme} --lite    → board-meeting + PRD（prd_file を記録）
  2. /ccc-plan-project {prd_file}         → impl-plan（impl_plan_file を記録）
  3. /ccc-implement-project --auto {impl_plan_file} → 実装→QA→デプロイ
...
```
/ccc-start-project {テーマi} --lite
```

- 既定は `--lite`。`--board=full` 指定時、または batch-themes.md の該当テーマに `board: full` がある場合のみフル取締役会
- 内部的に ccc-start-project の Phase 1〜6 が走る（アーカイブは B-4 で本コマンドが実施）
...
```
/ccc-plan-project {batch-state に記録した prd_file}
```
...
```
/ccc-implement-project --auto {batch-state に記録した impl_plan_file}
```

- `(latest)` 渡しは使用禁止
- 内部で `ccc-engineer` → `ccc-qa` → `ccc-devops` が auto モードで走る
- devops 起動前に該当テーマの state.md が `mode: auto` かつ `auto_approve: true`（Phase A-0 承認由来）であることを確認し、不足していれば Edit で補正する（無人実行で承認待ちに入らないため）
- デプロイ直前には ccc-devops の Step 2.5 セキュリティゲート（gitleaks + SAST）が必ず実行される。ブロック条件（gitleaks leak>0 / SAST CRITICAL>0）の [SECURITY_STOP] 時はテーマ i を `failed` として次テーマへ
...
| ユーザーから中断指示 | 進行中のファイル書き込み・現在の1タスクのみ安全に完了させ、新規フェーズ（特に ccc-devops デプロイ）には進まず batch-state に `interrupted` を記録して即停止（--resume で再開可） |
...
```
/ccc-autorun-batch --resume={batch_id}
```
